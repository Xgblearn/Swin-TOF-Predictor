...
def window_partition(x, window_size: int):
    """
    Split feature map into non-overlapping windows of window_size
    Args:
        x: (B, H, W, C)
    ...
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size,
                                                            C)  # 输出的是按照指定的window_size划分成一个一个窗口的数据
    return windows
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # padding
        # if H or W are not multiples of patch size, pad
        pad_input = (H % self.patch_size[0] != 0) or (W % self.patch_size[1] != 0)
        if pad_input:
            # to pad the last 3 dimensions,
            # (W_left, W_right, H_top,H_bottom, C_front, C_back)
            x = F.pad(x, (0, self.patch_size[1] - W % self.patch_size[1],  # right pad for width
                          0, self.patch_size[0] - H % self.patch_size[0],  # bottom pad for height
                          0, 0))
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        """
        x: B, H*W, C    channel-last sequence
        """
        B, L, C = x.shape
        ...
        # padding
        # if H or W are not even, pad
        pad_input = (H % 2 == 1) or (W % 2 == 1)
        if pad_input:
            # to pad the last 3 dimensions, starting from the last dimension and moving forward.
            # (C_front, C_back, W_left, W_right, H_top, H_bottom)
            # tensor layout is [B, H, W, C]
            x = F.pad(x, (0, 0, 0, W % 2, 0, H % 2))

        # sample with stride 2
...
"""
WindowAttention module (W-MSA / SW-MSA)
"""
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.2):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1),
                        num_heads))  # [2*Mh-1 * 2*Mw-1, nH]    其元素的个数===>>[(2*Mh-1) * (2*Mw-1)]
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # apply mask: same region 0, others -100
        if mask is not None:
            # mask: [nW, Mh*Mw, Mh*Mw]
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        # cyclic shift (SW-MSA if shift_size>0 else W-MSA)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size,
        ...
        self.shift_size = window_size // 2  # shift size
    ...
    def create_mask(self, x, H, W):
        ...
        # same layout as feature map for window_partition
        ...
        mask_windows = window_partition(img_mask,
                                        self.window_size)  # [nW, Mh, Mw, 1]           # 输出的是按照指定的window_size划分成一个一个窗口的数据
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size)  # [nW, Mh*Mw]
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)  # broadcasting
        # same region -> 0, others -> -100
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4,  # 表示通过Patch Partition层后，下采样几倍
                 ...
                 qkv_bias=True,
                 drop_rate=0.1,  # 对应的PatchEmbed层后面的
                 attn_drop_rate=0.1,  # 对应于Multi-Head self-Attention模块中对应的dropRate
        ...
        self.num_layers = len(
            depths)  # depths:表示重复的Swin Transoformer Block模块的次数  表示每一个Stage模块内，Swin Transformer Block重复的次数
        self.embed_dim = embed_dim
        ...
        self.avgpool = nn.AdaptiveAvgPool1d(1)  # global avg pool
        C = self.num_features
        C2 = C// 2
        C4 = C// 4
        self.mlp1 = nn.Sequential(
            nn.Linear(C, C2),  # C → C/2
            nn.GELU(),
            nn.Linear(C2, C4),  # C/2 → C/4
            nn.GELU(),
            nn.Linear(C4, C4),  # C/4 → C/4
            nn.GELU(),
            nn.Linear(C4, C2),  # C/4 → C/2
            nn.GELU(),
            nn.Linear(C2, C),  # C/2 → C
        )
        ...
        #     nn.Linear(C2, C),
        # )
        self.mlp_dropout = nn.Dropout(drop_rate)
    ...
    def forward(self, x):
        # x: [B, L, C]
        x, H, W = self.patch_embed(x)  # downsample by 4
        x = self.pos_drop(x)
        ...
        x = torch.flatten(x, 1)
        # optional residual MLP branch
        out1 = self.mlp1(x)  # [B, C]
        # out2 = self.mlp2(x)  # [B, C]
        # delta = self.mlp_dropout(out1 + out2)
        delta = self.mlp_dropout(out1)
        x = x + delta  # [B, C]
