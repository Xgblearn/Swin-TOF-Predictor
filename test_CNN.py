...
class MyDataset(Dataset):
    def __init__(self, target_dir, label_dir, signal_len=3000):
        self.target_dir = target_dir
        self.label_dir = label_dir
        self.signal_len = signal_len

        # Get all target files and label files
        ...
        self.targets = [os.path.join(target_dir, f"{name}.txt") for name in common_names]
        self.labels = [os.path.join(label_dir, f"{name}.txt") for name in common_names]
...
# CNN Model
# -------------------------
class Paper1DCNN(torch.nn.Module):
    def __init__(self, input_channels=1, signal_len=3000, out_dim=1, dropout=0.2):
        super().__init__()
        self.conv1 = torch.nn.Conv1d(input_channels, 4, kernel_size=9, stride=4, padding=3)
        self.bn1 = torch.nn.BatchNorm1d(4)
        self.conv2 = torch.nn.Conv1d(4, 8, kernel_size=9, stride=4, padding=3)
        ...
        self.fc1 = torch.nn.Linear(flat_dim, 400)
        self.dropout = torch.nn.Dropout(dropout)
        self.fc2 = torch.nn.Linear(400, out_dim)
...
def run_testing(target_dir, label_dir, model_path,
                batch_size=32, signal_len=3000, exp_name="model_test"):
    """
    ...
    save_dir = f"test_results_CNN/{exp_name}_{timestamp}"
    ...
    test_dataset = MyDataset(target_dir, label_dir, signal_len)
    ...
    # Save error histogram separately
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='1D CNN model testing script (supports non-continuous file names)')
    ...
    parser.add_argument('--model_path', default="saved_model/best_cnn_model.pth", type=str, help='Path to trained model')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--exp_name', type=str, default="cnn_15DB", help='Test experiment name')
